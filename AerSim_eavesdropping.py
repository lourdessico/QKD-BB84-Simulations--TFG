...
# In this code I simulate the BB84 protocol using qiskit with eavesdropping.
# I skip the step of discarding the bits in which the bases do not match since we dont 
# need those bits after all. So Alice and Bob will have the same bases and n= the number of bits after discarding
# I simulate the protocol s times with n qbits each time. Then I study the probability of detecting Eve (d/s), 
# with d the amount of times Eve is detected.
...
n = 48 # number of qbits after discarding
...
with open("Backend_Prob_eve_correct.txt", "a", encoding="utf-8") as f:
    ...
    f.write(f"Theoretical prob. of Eve measuring the right results:\t{0.75:.6f}\n")
    f.write(f"Theoretical prob. of Bob's measuring the wrong results:\t{0.25:.6f}\n")

with open("Backend_Detected eavesdropping.txt", "a") as f:
...
data = np.loadtxt("Backend_Detected eavesdropping.txt", skiprows=1)
...
plt.errorbar(ñ_values, probabilities, yerr=errors, fmt='ro', capsize=4, label='Simulated')
...
plt.grid(True)
plt.tight_layout()
plt.savefig("Backend_Prob_deteccion_eve_vs_ñ.png")
plt.show()
