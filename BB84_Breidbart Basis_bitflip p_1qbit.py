...
# Breidbart-like basis. Bitflip errors with probability p are applied. If p = 0 and beta = 0 
# the Breidbart  basis is recovered. Alice and Bob always use the same basis, so no bits are
# discarded. The simulation is repeated s times to study the probability of detecting Eve (d/s),
# the probability that Eve guesses correctly, and the error induced by her presence.
...
def f(angle):
    return 0.5*(np.cos(2.0*angle)*(0.5 - p) + 1.0 - 0.5*np.sin(2.0*angle))

# Theoretical probability of Bob measuring the wrong result
def g(angle): 
    return  (
    (2*(((1-p)**2)*(np.sin(angle)**2)*(np.cos(angle)**2) + (1-p)*p*(np.sin(angle)**4) + (1-p)*p*(np.cos(angle)**4) + (p**2)*(np.sin(angle)**2)*(np.cos(angle)**2)) +
     2*(0.25 - (np.sin(angle)**2)*(np.cos(angle)**2)))*0.5
)

# Objective function to minimize
def objective(angle, alpha, beta):
    return -alpha * f(angle) + beta * g(angle)
...
# angle = 0.5 * np.arctan(-1.0 / (1 - 2.0 * p))  # optimal eavesdropping basis angle maximizing f(angle) 

# print(f"Optimal angle: {optimal_angle:.6f} rad")
# print(f"f(opt) = {f(optimal_angle):.6f}, g(opt) = {g(optimal_angle):.6f}")
# print(f"Angle: {angle:.6f} rad")
# print(f"f(opt) = {f(angle):.6f}, g(opt) = {g(angle):.6f}")

simulator = AerSimulator()
bitflip_error = pauli_error([('X', p), ('I', 1 - p)])
...
Pc = 0.5*(np.cos(2.0*angle)*(0.5 - p) + 1.0 - 0.5*np.sin(2.0*angle)) # theoretical probability of Eve measuring the right result
P_err_bob= (
    (2*(((1-p)**2)*(np.sin(angle)**2)*(np.cos(angle)**2) + (1-p)*p*(np.sin(angle)**4) + (1-p)*p*(np.cos(angle)**4) + (p**2)*(np.sin(angle)**2)*(np.cos(angle)**2)) +
     2*(0.25 - (np.sin(angle)**2)*(np.cos(angle)**2)))*0.5
)
with open("Prob_eve_correct__Breidbart_error p_1qbit.txt", "a", encoding="utf-8") as f:
    f.write(f"n = {n}\tp = {p}\talpha = {alpha}\tbeta = {beta}\tangle = {angle}\n")
    f.write(f"Prob. Eve correct: ({np.mean(P_tot_eve_correct):.8f}\u00B1{sigma_error_eve:.8f})\n")
    f.write(f"Error caused by Eve and p: ({np.mean(P_tot_error_caused):.8f}\u00B1{sigma_error_caused:.8f})\n")
...
with open("Detected eavesdropping_Breidbart_error p_1qbit.txt", "a") as f:
    f.write(f"Initial length: {n*2}\tp = {p}\tbits compared: {size}\t Prob detecting eve: {d / s:.8f}\n")
