...
# Breidbart-like basis. Phaseshift errors with probability q are applied. If q = 0 and beta = 0 
...
def f(angle):
    return 0.5*(0.5+(2.0*q - 1.0)*(np.sin(angle)*np.cos(angle))+(np.cos(angle))**2)

# Theoretical probability of Bob measuring the wrong result
def g(angle): 
    return  (
    0.5*((2.0*(1.0 - q)*q*(0.5 + 2*((np.sin(angle))**2)*((np.cos(angle))**2))) + 2.0*(0.25 - ((np.sin(angle))**2)*((np.cos(angle))**2))*((1-q)**2 + q**2) +
         (2.0*((np.cos(angle))**2)*((np.sin(angle))**2)))
)
...
# angle = 0.5 * np.arctan(2.0*q - 1.0) # optimal eavesdropping basis angle maximizing f(angle) 

# print(f"Optimal angle: {optimal_angle:.8f} rad")
# print(f"f(opt) = {f(optimal_angle):.8f}, g(opt) = {g(optimal_angle):.8f}")
# print(f"Angle: {angle:.8f} rad")
# print(f"f(opt) = {f(angle):.8f}, g(opt) = {g(angle):.8f}")



simulator = AerSimulator()
bitflip_error = pauli_error([('Z', q), ('I', 1 - q)])
...
Pc = 0.5*(0.5+(2.0*q - 1.0)*(np.sin(angle)*np.cos(angle))+(np.cos(angle))**2) # theoretical probability of Eve measuring the right result
P_err_bob= (
    0.5*((2.0*(1.0 - q)*q*(0.5 + 2*((np.sin(angle))**2)*((np.cos(angle))**2))) + 2.0*(0.25 - ((np.sin(angle))**2)*((np.cos(angle))**2))*((1-q)**2 + q**2) +
         (2.0*((np.cos(angle))**2)*((np.sin(angle))**2)))
)

with open("Prob_eve_correct__Breidbart_error q_1qbit.txt", "a", encoding="utf-8") as f:
    f.write(f"n = {n}\tq = {q}\talpha = {alpha}\tbeta = {beta}\tangle = {angle}\n")
    f.write(f"Prob. Eve correct: ({np.mean(P_tot_eve_correct):.8f}\u00B1{sigma_error_eve:.8f})\n")
    f.write(f"Error caused by Eve and q: ({np.mean(P_tot_error_caused):.8f}\u00B1{sigma_error_caused:.8f})\n")
...
with open("Detected eavesdropping_Breidbart_error q_1qbit.txt", "a") as f:
    f.write(f"Initial length: {n*2}\tq = {q}\tbits compared: {size}\t Prob detecting eve: {d / s:.8f}\n")
